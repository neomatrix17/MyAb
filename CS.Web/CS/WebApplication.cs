...
public partial class AdressenManagementAspNetApplication : WebApplication
{
	...
	private void InitializeComponent()
	{
		...
		base.DatabaseVersionMismatch += AdressenManagementAspNetApplication_DatabaseVersionMismatch;
		this.LoggedOn += WebApplication_LoggedOn;
		this.LoggingOff += WebApplication_LoggingOff;
		this.LogonFailed += BauManager2015AspNetApplication_LogonFailed;
			...
			if (!(SecuritySystem.CurrentUser == null))
			{
				...
				if (!((AdressenManagement.Module.BusinessLogic.Intern.Mitarbeiter) SecuritySystem.CurrentUser).IsKunde.HasValue)
				{
					...
					try
					{
                        ...
                        CustomLog logRecord = os.CreateObject<AdressenManagement.Module.BusinessLogic.Basis.CustomLog>();
						
						logRecord.Benutzername = SecuritySystem.CurrentUserName;
						logRecord.BenutzerID = (short)SecuritySystem.CurrentUserId;
		...
		try
		{
			if (!(SecuritySystem.CurrentUser == null))
			{
				...
				if (!((AdressenManagement.Module.BusinessLogic.Intern.Mitarbeiter) SecuritySystem.CurrentUser).IsKunde.HasValue)
				{
					...
					{
						var s = (RecensaWatch) HttpContext.Current.Session["Watch" + System.Convert.ToString(SecuritySystem.CurrentUserId)];
						if (!(s == null))
						{
							recensaWatch = s;
						}
	...
	private void BauManager2015AspNetApplication_LogonFailed(object sender, DevExpress.ExpressApp.LogonFailedEventArgs e)
	{
		...
		try
		{
            ...
            var logonParameters = (DevExpress.ExpressApp.Security.AuthenticationStandardLogonParameters)e.LogonParameters;
            logRecord.Benutzername = logonParameters.UserName;
			logRecord.SystemName = this.ApplicationName;
			logRecord.Zeitstempel = DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss");
