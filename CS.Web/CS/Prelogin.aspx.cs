	...
	private void Prelogin_Init(object sender, EventArgs e)
	{
		...
		if (!IsPostBack)
		{
			...
			if (!(Request.Cookies["mabsession"] == null))
			{
				...
				{
					MaLogin.Value = tempData[0].ToString();
                    MaPass.Value = tempData[1].ToString();
					MaRememberMe.Checked = (bool)tempData[2];
				}
			}
		...
		if (MaLogin.Value != "" || MaPass.Value != "")
		{
			...
			if (!(lp == null))
			{
				...
				else
				{
					...
					{
						customerror.Text = DoTranslate(HttpContext.Current.Session["CustomError"].ToString());
